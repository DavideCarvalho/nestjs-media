...
- 8852c83: Resumable uploads can now carry **opaque application metadata** from `createUpload` through to the
  ...
  `upload.complete` told you only _that_ an object arrived at `disk`/`key` — not what it was for — so
