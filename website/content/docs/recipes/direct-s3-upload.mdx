...
The upload's `location` ends in the session id; the bytes are already at the `keyFor` path on the disk. Record it against your entity:

```ts
// after the client reports `location`, e.g. POST it to your API:
await media.library.attach({
  ownerType: 'Post', ownerId: postId, collection: 'gallery',
  fileName, mimeType, disk: 's3',
  contents: await media.disk('s3').stream(uploadedKey), // re-key into the library layout
});
```
