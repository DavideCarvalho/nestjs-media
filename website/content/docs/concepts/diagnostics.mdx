...
Each emitter takes `emitDiagnostics: false` to opt out (`MediaLibrary`, `ResumableUploadManager`, `AttachmentManager`). See [`@dudousxd/nestjs-diagnostics`](https://github.com/DavideCarvalho/nestjs-diagnostics) for the cross-ecosystem convention this follows.
...
### Running alongside the generic diagnostics bridge

If you also run `@dudousxd/nestjs-diagnostics-telescope`'s generic bridge, you don't need to hand-maintain an exclude list to avoid double-recording. `MediaWatcher.register()` claims every event it records via the diagnostics claim registry, so the generic bridge's watcher skips them automatically at record time — you get one Telescope entry per event, not two. The one thing that doesn't change: `upload.progress` isn't claimed (this watcher doesn't record it either), so if you don't want it flooding the generic bridge too, mute it there yourself with `mediaDiagnosticKey('upload.progress')` in that bridge's `exclude` option.
