...
icon: Layers
description: Storage (layer 1) is a disk-agnostic filesystem; media-library (layer 2) attaches files to entities on top of it. How they relate, and why they're split.
---
...
```ts
disk.capabilities; // { presign: boolean; multipart: boolean; publicUrls: boolean; list: boolean; ranged: boolean }
```

`publicUrls` gates `url()`, `presign` gates `temporaryUrl()` and is consulted by [`uploadMode`](/docs/concepts/uploads) to decide direct vs proxied uploads, `list` gates prefix enumeration, and `ranged` gates `stream(path, { start, end })` — the seek that lets the console read a page out of a multi-gigabyte object instead of transferring it. A driver that can't do something throws `UnsupportedOperationError` instead of silently faking it.
...
await post.list();          // every collection for this post
```

`url` and `delete` act on a record id (not an owner), so they stay on `media.library`.
...
The `media` table is polymorphic and managed by your chosen [ORM store](/docs/concepts/persistence) — TypeORM, MikroORM, Drizzle, or Prisma, all behind the same `MediaStore` contract.
